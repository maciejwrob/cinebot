...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const { embed, rows } = buildTopEmbed(results, '🎬 Filmy', 0xe74c3c, label, 0);
    ...
    collector.on('collect', async (btn) => {
      if (btn.customId.startsWith('info_select')) return; // obsługiwane w bot.js
      ...
      const { embed: newEmbed, rows: newRows } = buildTopEmbed(results, '🎬 Filmy', 0xe74c3c, label, currentPage);
